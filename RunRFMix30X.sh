#!/bin/bash
#SBATCH --job-name=RFMix_30X_submit
#SBATCH --partition=mono
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=4
#SBATCH --output=logRFMix30X/submit_%j.out
#SBATCH --error=logRFMix30X/submit_%j.err

set -euo pipefail

# This launcher submits one RFMix job per chromosome for hcWGS/30X data.
# It is intended to be run on the cluster with: sbatch RunRFMix30X.sh

BASE=${BASE:-/home/dsoria/PoblAR/LocalAncestryMuestrasPiloto}
INPUT=${INPUT:-${BASE}/Script/Input}
OUTDIR=${OUTDIR:-${BASE}/30X}
SINGULARITY_IMAGE=${SINGULARITY_IMAGE:-/home/dsoria/miniconda3/rfmix.sif}
BIND_PATH=${BIND_PATH:-/home/pluisi/}

QUERY_VCF=${QUERY_VCF:-${INPUT}/LocalAncestry.30X33IndYControl.vcf.gz}
REFERENCE_VCF=${REFERENCE_VCF:-${INPUT}/LocalAncestry.Referencias30X.vcf.gz}
...
for chr in $(seq 1 22); do
  ...
  sbatch <<EOF
#!/bin/bash
#SBATCH --job-name=RFMix30X_${chr_name}
#SBATCH --partition=mono
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=4
