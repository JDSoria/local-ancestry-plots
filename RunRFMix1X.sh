...
# This launcher submits one RFMix job per individual and chromosome for lcWGS/1X data.
...
INDIVIDUALS_FILE=${INDIVIDUALS_FILE:-${INPUT}/33Ind+Control.txt}
...
[[ -f "${INDIVIDUALS_FILE}" ]] || { echo "Missing individuals file: ${INDIVIDUALS_FILE}" >&2; exit 1; }
...
while read -r individual; do
  [[ -n "${individual}" ]] || continue

  for chr in $(seq 1 22); do
    chr_name="chr${chr}"
    map_file="${GENETIC_MAP_DIR}/${chr_name}.b38.gmap.final.corrected.modified"
    out_prefix="${OUTDIR}/${individual}/${chr_name}/cromosomaPPieFinal${chr}_${individual}"
    ...
    sbatch <<EOF
#!/bin/bash
#SBATCH --job-name=RFMix1X_${chr_name}_${individual}
#SBATCH --partition=mono
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=4
